...
public class WeeklyInterviewScheduler {
    ...
    //주간 면접왕
    //Mon 00:00:00 매주 -> 재실행시 실행되므로, Now() -> 날짜계산 해서 쿼리
    ...
    public void runWeeklyInterviewJob() throws JobParametersInvalidException, JobExecutionAlreadyRunningException, JobRestartException, JobInstanceAlreadyCompleteException {
        ...
        try {
            ...
            log.info("WEEKLY INTERVIEW SCHEDULER >> 배치 실행됨 >> Time: {}", Calendar.getInstance().getTime());
        } catch (JobExecutionAlreadyRunningException |
                JobInstanceAlreadyCompleteException |
                JobParametersInvalidException |
                JobRestartException e) {
            log.error("WEEKLY INTERVIEW SCHEDULER >> 배치 실패됨 >> Time: {} Error: {}", Calendar.getInstance().getTime(), e.getMessage());
        }
    }
}
